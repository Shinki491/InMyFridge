...
function normalizeRecipe(raw: any) {
  return {
    ...
    instructions:
      raw.instructions ??
      (Array.isArray(raw.analyzedInstructions) && raw.analyzedInstructions.length > 0
        ? raw.analyzedInstructions[0].steps?.map((s: any) => s.step).join(" ")
        : ""),
  };
}
...
export async function getRecipeDetails(apiId: number) {
  if (!isMongoReady()) {
    ...
    return { source: "spoonacular" as const, recipe: normalizeRecipe(fresh) };
  }
  ...
  if (cached) {
    ...
    if (ageMs < ttlMs) {
      return { source: "cache" as const, recipe: normalizeRecipe(cached.payload) };
    }
  }
